...
function openSpreadsheetByKey_(propertyKey) {
  var spreadsheetId = PropertiesService.getScriptProperties().getProperty(propertyKey);

  if (!spreadsheetId) {
    throw new Error("Script Properties에 '" + propertyKey + "' 값이 없습니다.");
  }
...
function getSourceSheets_(spreadsheet) {
  return spreadsheet.getSheets().filter(function(sheet) {
    var headerKeys = getHeaderKeys_(sheet);
    return headerKeys.indexOf("word_id") >= 0 && headerKeys.indexOf("jp_kanji") >= 0;
  });
}

function getActiveSourceRows_(spreadsheet) {
  var sheets = getSourceSheets_(spreadsheet);
  var rows = [];
...
function buildQuestionDtosFromSourceRows_(sourceRows) {
